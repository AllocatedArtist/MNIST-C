...
void mnist_read_test_data(const char *path, mnist_t *mnist) {
  ...
  assert(dimensions_count != 0 && "can't read empty data");
  ...
  uint8_t *all_bytes = (uint8_t *)malloc(size);
  ...
  mnist->testing_data_ = (mnist_input *)malloc(sizeof(mnist_input) * img_count);

  for (int i = 0; i < img_count; ++i) {
    image_t *current = &mnist->testing_data_[i].image_;

    current->data_ = (uint8_t *)malloc(PIXEL_COUNT);
...
void mnist_shuffle(mnist_t *mnist) {
  ...
  mnist_input *shuffled_array =
      (mnist_input *)malloc(sizeof(mnist_input) * size);
  for (int i = 0; i < size; ++i) {
